...
export default async function HomePage() {
  ...
  return (
    <div>
      {/* ── Hero: portrait + intro ─────────────────────────────── */}
      <section className="mx-auto max-w-5xl px-5 pb-12 pt-14 md:px-8 md:pb-20 md:pt-20">
        <div className="flex flex-col gap-10 md:flex-row md:items-start md:gap-16">
          {/* Avatar */}
          <div className="animate-rise flex-shrink-0">
            <div className="relative h-40 w-40 overflow-hidden rounded-full border-4 border-white shadow-[var(--shadow)] md:h-48 md:w-48">
              {config.avatar ? (
                <Image
                  src={config.avatar}
                  alt={config.fullName}
          ...
          <div className="animate-rise-delay">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                href="/projects"
                className="rounded-full bg-accent px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-accent-bright"
              >
                Projects
              </Link>
              <Link
                href="/blog"
                className="rounded-full border border-ink/15 bg-white/60 px-5 py-2.5 text-sm font-semibold text-ink transition hover:border-accent hover:text-accent"
              >
                Writing
              </Link>
              <Link
